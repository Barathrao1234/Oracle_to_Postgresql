...
(omit this section entirely only if truly empty)

1. **<construct/location, e.g. "TABLE(...) in subquery `q2`, line 14">**
   - Why it couldn't be fully resolved: <reason>
   - What was done instead: <ASSUMED best-effort form used, or "left as UNRESOLVABLE INLINE fragment">
...
## Final Validation
- Oracle-specific syntax scan: PASS (all remaining instances are inside marked Manual Follow-Up fragments) | FAIL
- Semantic review: PASS | PASS WITH NOTED GAPS (see Manual Follow-Up)
...
6. Every assumption or unresolved fragment must appear both inline (comment) and in the `Manual Follow-Up Required` list, with what's needed to finish it.
7. Prefer flagging a gap over guessing incorrectly — but flag-and-continue, not stop-and-ask.
